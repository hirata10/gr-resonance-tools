...
double J_dot_tidal(int nl, int n_res_inner, int n_res_outer, int k_res_inner, int k_res_outer, int m_res_inner, int m_res_outer, double apo, double rp, double radius_outer, double I, double M, double astar, double *J_dot_r_tidal, double *J_dot_theta_tidal, double *J_dot_phi_tidal){
	...
	*J_dot_phi_tidal = 0;  	
	...
	CKerr_RadialFunc(Minv_outer, xuorig_outer, M, astar, n_res_outer, k_res_outer, m_res_outer, 1, 1, nl, C0_outer, &omegagw_outer, E0_outer);
	...
	for (i = -1; i <= 2; i++){
		...
		for (il = 0; (il) < nl; il++){
			...
			printf("%i \t %i \t %i \t %i \t %lg \t %lg \t %lg \t %lg \t %lg \t %lg \t %lg \t %lg\n", i_n, i_k, i_m, il,  C0_inner[4*il], C0_inner[4*il+1], C0_inner[4*il+2], C0_inner[4*il+3], C0_outer[4*il], C0_outer[4*il+1], C0_outer[4*il+2], C0_outer[4*il+3]);
			//printf("%i \t %i \t %i \t %i \t %lg \t %lg \t %lg \t %lg \t %lg \t %lg\n", i_n, i_k, i_m, il, C0[4*il], C0[4*il+1], C0[4*il+2], C0[4*il+3], Z_down_square, Z_out_square);

			/* Expanded out the terms in our expression for \dot{J}_{td} and kept only the real parts*/
			term = C0_outer[4*il+2] * cscat[0] * Rtheta + C0_outer[4*il+3] * cscat[0] * C0_inner[4*il] * Itheta + C0_outer[4*il+2] * cscat[1] * C0_inner[4*il+1] * Rtheta + C0_outer[4*il+3] * cscat[1] * C0_inner[4*il+1] * Itheta;
			another_term = -C0_outer[4*il+2] * C0_inner[4*il+1] * cscat[0] * Itheta + C0_outer[4*il+3] * C0_inner[4*il+1] * cscat[0] * Rtheta + C0_outer[4*il+2] * cscat[1] * C0_inner[4*il] * Itheta - C0_outer[4*il+3] * cscat[1] * C0_inner[4*il] * Rtheta;
			last_term = C0_outer[4*il+2] * C0_inner[4*il+2] * Rtheta + C0_outer[4*il+3] * C0_inner[4*il+2] * Itheta - C0_outer[4*il+2] * C0_inner[4*il+3] * Itheta + C0_outer[4*il+3] * C0_inner[4*il+3] * Rtheta;
			printf("Real and Imgainary parts of c13 = %lg %lg \n", cscat[0], cscat[1]);
			/* J_dot of inner body due to tidal field of outer body */
			*J_dot_r_tidal += -i_n * (term + another_term + alphankm * last_term) / (2 * omega_nkm * omega_nkm * omega_nkm);
			*J_dot_theta_tidal += -i_k * (term + another_term + alphankm * last_term) / (2 * omega_nkm * omega_nkm * omega_nkm);
			*J_dot_phi_tidal += -i_m * (term + another_term + alphankm * last_term) / (2 * omega_nkm * omega_nkm * omega_nkm);
				}
			}
	free((char*)E0_inner);
	free((char*)E0_outer);
