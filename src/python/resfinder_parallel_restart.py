        ...
        for line in f_check:
            ...
            if len(parts) >= 14:
                try:
                    ...
                    J_inner_key = tuple(round(float(parts[i]), 5) for i in [13,14,15])
                    J_outer_key = tuple(round(float(parts[i]), 5) for i in [16,17,18])
                    combo_key = (m_i, n_inner, k_inner, n_outer, k_outer, J_inner_key, J_outer_key)
                    completed_combinations.add(combo_key)   
                    # combo_key = (n_inner, n_outer, k_inner, k_outer)

                    if not (isinstance(m_i, int) and isinstance(n_inner, int) and isinstance(k_inner, int) and isinstance(n_outer, int) and isinstance(k_outer, int)):
                        print(f"Unexpected non-integer values in combo_key: {combo_key}")
...
# # Fixed n_sub for a varying time grid
# print(len(time_value_1))
# sys.exit()
...
# # Vary n_sub to nearly fix number of grid points in time axis
# Target total number of time points
target_length = globalpars.GLOBALPAR_tgrid_resfinder

# Define a function that creates nsub subsections within intervals
def subsample(X, nsub):
    L = len(X)
    Xsub = np.zeros((L - 1) * nsub + 1)
    for j in range(L - 1):
...
num_intervals = len(time_value_1) - 1

# Compute the best n_sub to get closest to target_length
n_sub = max(1, round((target_length - 1) / num_intervals))

# Generate the subsampled time grid
t = subsample(time_value_1, n_sub)
total_steps = len(t)

print(f"Desired number of time points in grid: {target_length}")
print(f"Length of time array: {len(time_value_1)}")
print(f"Number of subsamples: {n_sub}")
print(f"Total time points in grid: {total_steps}")
                                    ...
                                    if gd < 3 and ispos([m_i, n_inner, n_outer, k_inner, k_outer]):
                                        ...
                                        if gd == 1 or k_checker != 0:
                                            if t[i] != time_value_1[0]:
                                                # f.write(f"[DEBUG] i={i}, m[i]={m[i]}, m[i+1]={m[i+1]}, diff={abs(m[i+1] - m[i])}\n")
                                                ...
                                                J_inner_key = tuple(round(float(val), 5) for val in J_inner_res)
                                                J_outer_key = tuple(round(float(val), 5) for val in J_outer_res)
                                                combo_key = (m_i, n_inner, k_inner, n_outer, k_outer, J_inner_key, J_outer_key)
