...
double Omega_outer_direct(double radius, double M, double spin){
	...
	u = 1./radius;
	omegaouter = sqrt(M*u*u*u)/(1. + spin*sqrt(M*u*u*u));
...
double ra_rp_I2Omega(int n, int k, int m, double radius, double ra, double rp, double I, double astar, double M){
	double EQL[3];
	double J[3];
	double Minv[9];
...
/* Bisection method for finding the apocenter of the resonance condition */
double find_resonance_apo(int n, int k, int m, double radius, double guess1, double guess2, double rp, double I, double astar, double M){
	double mid_apo=0;
	double error=0;
	int i=0;
	double mold=0;
	...
	if(ra_rp_I2Omega(n, k, m, radius, guess1, rp, I, astar, M)*ra_rp_I2Omega(n, k, m, radius, guess2, rp, I, astar, M)>0){
        	printf("Value at first and second guess: %lf, %lf \n", ra_rp_I2Omega(n, k, m, radius, guess1, rp, I, astar, M), ra_rp_I2Omega(n, k, m, radius, guess2, rp, I, astar, M));
        	printf("Invalid Interval Exit! \n");       //to test whether search interval
        	exit(1);                                                        //is okay or not
	}
	else if(ra_rp_I2Omega(n, k, m, radius, guess1, rp, I, astar, M)==0 || ra_rp_I2Omega(n, k, m, radius, guess2, rp, I, astar, M)==0){
        	printf("Value at first and second guess: %lf, %lf \n", ra_rp_I2Omega(n, k, m, radius, guess1, rp, I, astar, M), ra_rp_I2Omega(n, k, m, radius, guess2, rp, I, astar, M));
        	printf("Root is one of interval bounds. Root is %f\n",ra_rp_I2Omega(n, k, m, radius, guess1, rp, I, astar, M)==0?guess1:guess2);
        	exit(0);
	}
